...
if opts.use_sar and opts.use_input2 is False:
    ...
    meta_learner = MemoryNet_C(in_c=13 + 2, n_feat=64, scale_unetfeats=32).to(device)
else:
...
for epoch in range(num_epochs):
    ...
    for i, images in enumerate(train_dataloader):
        ...
        if opts.use_sar is not None and opts.use_input2 is None:
            ...
            outputs = meta_learner(cloudy, sars)
        else:
        ...
        if opts.use_rgb:
            ...
            targets_rgb = targets
        cloudy_attn = mask.repeat(1, outputs[0].size(1), 1, 1)
        loss_char = torch.sum(
