...
class SPANet(nn.Module):
    def __init__(self, in_s1=2, in_s2=3, out_c=3, feature_c=32):
        super(SPANet, self).__init__()
        self.conv_in_s1 = nn.Sequential(
            conv3x3(in_s1, feature_c // 2),
            nn.BatchNorm2d(feature_c // 2, affine=True),
            nn.ReLU(True),
        )
        self.conv_in_s2 = nn.Sequential(
            conv3x3(in_s2, feature_c // 2),
            nn.BatchNorm2d(feature_c // 2, affine=True),
            nn.ReLU(True),
        )
        ...
        self.res_block11 = Bottleneck(feature_c, feature_c)
        self.res_block12 = Bottleneck(feature_c, feature_c)
        self.res_block13 = Bottleneck(feature_c, feature_c)
        self.res_block14 = Bottleneck(feature_c, feature_c)
        self.res_block15 = Bottleneck(feature_c, feature_c)
        self.res_block16 = Bottleneck(feature_c, feature_c)
        self.res_block17 = Bottleneck(feature_c, feature_c)
        self.conv_out = nn.Sequential(
            conv3x3(feature_c, out_c)
        )

    def forward(self, s1, s2):
        s1 = self.conv_in_s1(s1)
        s2 = self.conv_in_s2(s2)
...
if __name__ == '__main__':
    torch.save(SPANet(2, 26, 3, 128).state_dict(), 'model_test/spanet.pth')
